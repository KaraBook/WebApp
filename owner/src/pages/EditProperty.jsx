...
const EditProperty = () => {
  ...
  return (
    <div className="p-3 w-full mx-auto">
      ...
      {/* Stepper */}
      <div className="flex items-center space-x-0 overflow-x-auto w-full mt-10 mb-8">
            ...
            <div key={step.id} className="flex items-center">
              <button
                ...
                className={`w-8 h-8 flex items-center justify-center rounded-full text-sm font-medium border-2
                  ${completed ? "bg-black text-white border-black" : isCurrent ? "bg-black text-white" : "border-gray-300 text-gray-400"}`}
              >
                {completed ? <Check size={16} /> : step.id}
              </button>
              {index !== formSteps.length - 1 && <div className="h-0.5 w-[10%] bg-gray-300 mx-2" />}
            </div>
          );
        })}
      </div>
