...
import { CalendarCheck, Heart, User as UserIcon, Star, LifeBuoy, LogOut } from "lucide-react";
      ...
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        ...
        <div className="flex items-center gap-3">
          {user ? (
              ...
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-2 rounded-full px-2 py-1 hover:bg-muted transition-colors">
                  <Avatar className="h-8 w-8 md:h-9 md:w-9">
                    ...
                    <AvatarFallback>{(user?.name?.[0] || user?.mobile?.[0] || "U").toUpperCase()}</AvatarFallback>
                  </Avatar>
            ...
            <Button onClick={onLoginClick} className="text-sm">Login</Button>
          )}
        </div>
      </div>
    </header>
