...
<>
  <div className="border rounded-lg p-4 flex gap-3">
    <img src={booking.property.coverImage} className="w-20 h-16 object-cover rounded" />
    <div>
      <p className="font-semibold">{booking.property.propertyName}</p>
    ...
    {reasons.map(r => (
      <div key={r}
        ...
        className={`border p-3 rounded cursor-pointer ${
          reason===r ? "border-red-500 bg-red-50" : ""
        }`}>
        {r}
      </div>
    ))}
...
<>
  ...
  <div className="space-y-1 text-sm">
    <div className="flex justify-between">
      <span>Booking</span>
      <span>{booking.property.propertyName}</span>
    </div>
    <div className="flex justify-between">
      <span>Reason</span>
      <span>{reason}</span>
...
<div className="flex justify-between pt-4">
  ...
  <Button
    ...
    onClick={async()=>{
      if(step===2 && !preview) await loadPreview();
